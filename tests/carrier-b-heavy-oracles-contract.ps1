...
$stalled = @($valid)
$stalled[5] = Row 'ship_loadout:g0' 6 'x4:ship:1' 3
$cases = @([pscustomobject]@{ name = 'reset_member_family'; trace = $reset }, [pscustomobject]@{ name = 'missing_capture'; trace = @($valid | Where-Object { $_.value.section_revision -ne 4 }) }, [pscustomobject]@{ name = 'later_member_starvation'; trace = $stalled })
