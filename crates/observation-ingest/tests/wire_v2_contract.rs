#![expect(
    clippy::expect_used,
    reason = "invalid contract fixtures must fail immediately"
)]

use observation_ingest::{CompleteMessage, EnvelopeDecodeError, decode_complete_message};
...
fn envelope_v1_and_unknown_versions_fail_closed() {
    for version in [1, 3] {
        let fixture = String::from_utf8(START_V2.to_vec())
            ...
            .replacen("\"contract_version\":2", &format!("\"contract_version\":{version}"), 1);
        assert_eq!(
