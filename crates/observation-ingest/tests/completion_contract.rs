...
use observation_domain::{
    ...
    DigestAlgorithmVersion, EntityId, EnvelopeRecord, ImmutableBatchEnvelope,
    ObservationPolicyVersion, ObservationSchemaVersion, ObservationVersion,
    ProducerIncarnationId, RecordId, SectionAvailability, SectionCompletionEnvelope,
    SectionCoverage, SectionFreshness, SectionKey, SectionQuality, SectionRevisionId,
    SectionStartEnvelope, SectionState, SourceScopeId, TransportEpoch,
};
...
fn key(raw: &str) -> SectionKey { value(raw, SectionKey::new) }
...
fn current() -> CompletionCurrent {
    CompletionCurrent::new(BTreeMap::from([(key("sectors"), revision(4))]), Some(revision(6)))
}
fn staged() -> GenerationStager {
    ...
    assert_eq!(
        stager.stage_section_batch(batch("batch:1", "ship:2", "record:2"), b"batch-two", 20, 1, 2),
        ReceiverDisposition::Received
    );
    assert_eq!(
        stager.stage_section_batch(batch("batch:2", "ship:1", "record:1"), b"batch-one", 20, 1, 3),
        ReceiverDisposition::Received
    );
    stager
}
...
fn exact_completion_yields_one_immutable_unpublished_revision() {
    ...
    let certificate = stager.completion_certificate(completion()).expect("candidate exists");
    let CompletionOutcome::Validated(revision) = stager.complete_section(certificate, &current(), 4) else {
...
fn any_count_length_digest_or_version_mismatch_discards_candidate() {
    let mut stager = staged();
    let mut certificate = stager.completion_certificate(completion()).expect("candidate exists");
...
fn coverage_and_capture_evidence_are_preserved_without_publishing() {
    ...
    assert_eq!(revision.context().capture_window(), CaptureWindow::new(10, 20).unwrap());
