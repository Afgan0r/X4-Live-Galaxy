...
fn validate_batch(
    ...
    if v.records.len() != 1 || v.optional_detail.is_some() || v.section_ordinal == 0 {
        return Err(rejected());
    }
    let record = v.records.first().ok_or_else(rejected)?;
    ...
    if fields.next().is_some()
        ...
        || record.observation_version.get() != v.section_revision.get()
    {
        return Err(rejected());
    }
    Ok(())
