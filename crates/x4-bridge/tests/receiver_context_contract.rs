...
fn production_bytes_restore_eligibility_after_producer_reincarnation() {
    ...
    let reconnect_start = String::from_utf8(start_bytes("runtime-clock"))
        .expect("fixture is UTF-8")
    ...
    assert_eq!(
        replacement.submit_received(
            ...
            observation_domain::BatchId::new("outer:reconnect-start").expect("identity"),
            reconnect_start,
            0,
            3,
        ),
    ...
    let DecisionEligibility::Eligible(reconnected) =
        replacement.decision_eligibility(std::slice::from_ref(&key), 3, 10)
    else {
        panic!("transport-only reconnect must preserve the durable current revision");
    };
    assert_eq!(
        reconnected
            .revisions()
    ...
    let boundary_start = String::from_utf8(start_bytes("runtime-clock"))
        ...
        .replacen("\"transport_epoch\":1", "\"transport_epoch\":3", 1)
        .replacen("\"section_revision\":1", "\"section_revision\":2", 1)
        .replacen("\"quality\":\"unknown\"", "\"quality\":\"fresh\"", 1)
        .replacen(
            "\"coverage\":\"complete\"",
    ...
    assert_eq!(
        replacement.submit_received(
            observation_domain::TransportEpoch::new(3).expect("epoch"),
    ...
    let DecisionEligibility::Eligible(set) =
        replacement.decision_eligibility(std::slice::from_ref(&key), 5, 10)
    else {
